...
namespace PTTI.TradeTrainingSDK.Tools
{
    ...
    [RequireComponent(typeof(ToolContactSurface))]
    public class MetalSheet : MonoBehaviour
{
    ...
    {
        var surface = GetComponent<ToolContactSurface>();
        if (surface == null) surface = gameObject.AddComponent<ToolContactSurface>();

        var rb = GetComponent<Rigidbody>();
        if (rb == null) rb = gameObject.AddComponent<Rigidbody>();
        rb.isKinematic = true; // freeze it by default so it doesn't get pushed by jigsaw

        var grab = GetComponent<XRGrabInteractable>();
        if (grab == null) grab = gameObject.AddComponent<XRGrabInteractable>();

        // Ensure grab uses kinematic movement so it doesn't glitch through things
